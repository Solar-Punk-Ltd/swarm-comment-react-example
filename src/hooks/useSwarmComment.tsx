...
import {
  ...
  SwarmComment,
  CommentSettings,
} from "@solarpunkltd/swarm-comment-js";
...
export const useSwarmComment = ({ user, infra }: CommentSettings) => {
  ...
  const addMessage = useCallback((newMessage: VisibleMessage) => {
    setMessages((prevMessages) => {
      ...
      if (existingIndex !== -1) {
        ...
        return commentRef.current?.orderMessages
          ? commentRef.current.orderMessages(updated)
          : updated;
      }

      const newMessages = [...prevMessages, newMessage];
      return commentRef.current?.orderMessages
        ? commentRef.current.orderMessages(newMessages)
        : newMessages;
    });
  }, []);
  ...
  const sendReaction = useCallback((targetMessageId: string, emoji: string) => {
    return commentRef.current?.sendMessage(
      emoji,
